...
ApplyAnimationEx(playerid, animlib[], animname[], Float:fDelta, loop, lockx, locky, freeze, time)
{
	...
	if(loop > 0 || freeze > 0)
	{
		...
		// if(InPoint[playerid] == 1 && PlayerInfo[playerid][pAdminDuty] == 0)
		// {
		// 	new string[256];
		// 	format(string, sizeof(string), "%s using animation inside a point", GetRPName(playerid));
...
CMD:animlist(playerid, params[])
{
	...
	SendClientMessage(playerid, COLOR_GRAD4, "/point /tired /injured /dodge /scratch /what /come /hitch /shifty /traffic /ghands");
...
{
    if(!PlayerUseAnims(playerid)) return SendClientMessage(playerid, COLOR_ERROR, "[ERROR]{ffffff} You're currently unable to use animations at this moment.");
...
{
    if(!PlayerUseAnims(playerid)) return SendClientMessage(playerid, COLOR_ERROR, "[ERROR]{ffffff} You're currently unable to use animations at this moment.");
    ApplyAnimationEx(playerid, "DODGE", "Crush_Jump", 4.0, 0, 0, 0, 0, 0);
    return 1;
}
...
CMD:hu(playerid)
{
	return callcmd::handsup(playerid);
}
